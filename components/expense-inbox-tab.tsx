...
/**
 * 受領BOX タブ／ページで共通使用するメイン UI。
 * 経費入力ページ（/expenses）の「受領BOX」タブ内に組み込まれる。
 *
 * @prop companyId 表示対象の会社ID
 * @prop onAddNew 「+ 新規請求書」ボタンクリックで呼ばれる
 */
export function ExpenseInboxTab({
  companyId,
  onAddNew,
}: {
  companyId: string
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (partnerSearch.trim()) filters.partnerSearch = partnerSearch.trim()
      if (summarySearch.trim()) filters.summarySearch = summarySearch.trim()
      ...
      const [expResult, userProfile] = await Promise.all([
        getExpenseBoxItems(companyId, filters),
  ...
  }, [companyId, receivedDatePreset, receivedDateFrom, receivedDateTo, evidenceFilter, partnerSearch, summarySearch, scheduledDateFrom, scheduledDateTo, showReady, page])
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    setActionLoading(id)
    try {
      await updateTransactionStatus(id, companyId, newStatus as never)
      await loadData()
    } catch (e) {
  ...
  const handleToggleEvidenceNotRequired = async (id: string, value: boolean) => {
    setActionLoading(id)
    try {
      await setEvidenceNotRequired(id, companyId, value)
      await loadData()
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="space-y-1">
              ...
              <Input className="h-8 text-sm" placeholder="部分一致検索" value={partnerSearch} onChange={(e) => setPartnerSearch(e.target.value)} />
            </div>
            <div className="space-y-1">
              <Label className="text-xs">摘要</Label>
              <Input className="h-8 text-sm" placeholder="部分一致検索" value={summarySearch} onChange={(e) => setSummarySearch(e.target.value)} />
            </div>
            <div className="space-y-1">
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="flex items-center gap-2">
              <Checkbox id="showReady" checked={showReady} onCheckedChange={(v) => setShowReady(v === true)} />
      ...
      {/* 証憑メタ検索 */}
      <EvidenceSearch companyId={companyId} />

      {/* テーブル */}
      {loading ? (
        <Card><CardContent className="py-8"><p className="text-center text-muted-foreground">読み込み中...</p></CardContent></Card>
      ) : (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              ...
              {isAdmin && expenses.some(e => e.status === "READY") && (
                <span className="ml-2 text-sm font-normal text-muted-foreground">管理者は確定操作が可能です</span>
              )}
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                {expenses.length === 0 ? (
                  ...
                  expenses.map((exp) => (
                    <TableRow key={exp.id} className={isRecentlyUpdated(exp.latestEvidenceAt) ? "bg-blue-50 dark:bg-blue-950/30" : ""}>
                      <TableCell className="whitespace-nowrap font-mono text-sm">{formatDD(exp.scheduledDate)}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="whitespace-nowrap text-sm">
                        <Input
                          ...
                          onChange={async (e) => {
                            try {
                              await updateTransaction(exp.id, companyId, { receivedDate: e.target.value || null })
                              await loadData()
                            } catch (err) {
                      ...
                      <TableCell>
                        {exp.evidenceNotRequired ? (
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {isAdmin && (
                              <Button variant="ghost" size="sm" className="h-5 text-[10px] px-1" onClick={() => handleToggleEvidenceNotRequired(exp.id, true)}>
                                なしOK
                              </Button>
                            )}
                          </div>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          {exp.status === "DRAFT" && (
                            <Button variant="ghost" size="sm" className="h-7 text-xs" disabled={actionLoading === exp.id} onClick={() => handleStatusChange(exp.id, "READY")}>
                              準備完了
                            </Button>
                          )}
                          {exp.status === "READY" && (
                            <>
                              <Button variant="ghost" size="sm" className="h-7 text-xs" disabled={actionLoading === exp.id} onClick={() => handleStatusChange(exp.id, "DRAFT")}>
                                差戻し
                              </Button>
                              {isAdmin && (
                                <Button variant="ghost" size="sm" className="h-7 text-xs" disabled={actionLoading === exp.id} onClick={() => handleStatusChange(exp.id, "CONFIRMED")}>
                                  確定
                                </Button>
                              )}
                            </>
